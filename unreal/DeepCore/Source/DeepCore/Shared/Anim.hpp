...
/// Named kPi rather than PI because PI is a macro in several widely-used headers
/// (Unreal defines one), and a bare `const float PI` silently fails to compile there.
...
inline LegPose WalkLeg(float phase, float stride = 26.0f)
{
	...
	if (phase < 0.5f) {
		...
		p.knee = 4.0f + 8.0f * std::sin(t * kPi);   // a little give under load
		p.lift = 0.0f;
	}
	else {
		...
		p.knee = 42.0f * std::sin(t * kPi);
		p.lift = 0.16f * std::sin(t * kPi);
	}
	return p;
}
...
{
	return -std::abs(std::sin(Wrap01(phase) * kTau)) * amount;
}
...
{
	return std::sin(Wrap01(phase) * kTau - kPi * 0.5f) * amount;
}
...
{
	return std::sin(Wrap01(phase) * kTau + kPi) * amount;
}
...
inline IkResult TwoBoneIK(float targetDist, float upperLen, float lowerLen)
{
	...
	r.upper = std::acos(Clamp(cosUpper, -1.0f, 1.0f)) * 180.0f / kPi;
	r.lower = 180.0f - std::acos(Clamp(cosKnee, -1.0f, 1.0f)) * 180.0f / kPi;
	return r;
}
